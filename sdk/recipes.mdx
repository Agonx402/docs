---
title: "Recipes"
description: "End-to-end flows with @agonx402/sdk — register, fund, sign, settle, withdraw."
---

This page is the SDK-native version of [Your First Payment](/getting-started/first-payment). Everything is expressed in terms of [`@agonx402/sdk`](https://www.npmjs.com/package/@agonx402/sdk) helpers. If you want the low-level protocol walkthrough, start there.

<Info>
All examples target the live devnet deployment (program `3UyUFeNsUYPpM6hMRf7H8wg3MKEXQ82rqnsXhZrUwgSD`, chain ID `1`). Replace the `programId` and `AGON_CHAIN_IDS.*` value for other environments.
</Info>
...
```ts
// Resolve this from TokenRegistry or AGON_PROTOCOL_DEVNET_USDC_TOKEN_ID.
...
See [Authorized settler](/core-concepts/authorized-settler) for the full trust model, including the difference between `authorized_signer` and `authorized_settler` (a per-message optional field emitted by `createCommitmentMessage`).
