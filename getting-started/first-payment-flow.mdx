...
This guide shows the smallest useful Agon integration: one payer, one payee, one token, one payment channel, and one settled `agon-cmt-v4` message.

The examples below mirror the real V4 protocol tests. Until a dedicated SDK lands, this is the most direct way to integrate Agon.
...
One one-way payment channel exists for each `payer_id + payee_id + token_id` relationship. In V4, these channels are permanent.
