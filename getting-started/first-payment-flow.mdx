...
description: "A concrete builder guide for registering, funding, opening a lane, signing a commitment, and settling it."
---

This guide shows the smallest useful Agon integration: one payer, one payee, one token, one lane, and one settled `agon-cmt-v4` message.

The examples below use the generated Anchor client and mirror the real protocol tests in the V4 repo. Until a dedicated SDK lands, this is the most direct way to integrate the protocol.
...
You will use the same PDA scheme everywhere in V4, so it is worth defining these helpers once.
...
At this point you have two stable participant ids:
...
Agon settles only allowlisted tokens. After the payer has a normal SPL token account, it can deposit into protocol balance.
...
That moves tokens from the payer's token account into the protocol vault and credits the payer's `available_balance` inside `ParticipantAccount`.

## 5. Create The Lane

One directed lane exists for one `payer_id + payee_id + token_id` relationship. In V4, lanes are permanent.
...
The `payeeOwner` signer is only needed when the payee's inbound-lane policy requires consent. If the payee has switched to `Permissionless`, lane creation does not need the payee to sign.

## 6. Optionally Lock Funds To The Lane

Locked funds are not required for every relationship, but they are useful when the payee wants stronger guarantees.
...
When settlement later happens, the program spends locked balance first and shared participant balance second.

## 7. Build The `agon-cmt-v4` Message

`agon-cmt-v4` is a cumulative message. It does not say "pay 25 again." It says "this lane is now authorized up to cumulative amount X."
...
Now build the first signed commitment:
...
The signer above must match the lane's current `authorized_signer`. By default, that is the payer wallet that created the lane. If the lane uses a different signing key, that key must sign here instead.
...
The program verifies the Ed25519 instruction, parses the message, checks the deployment message domain, checks the canonical payer, payee, token, and lane, then moves the delta between the old cumulative amount and the new cumulative amount.
...
That is the entire direct path.
