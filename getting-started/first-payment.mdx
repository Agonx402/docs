...
- Anchor workspace wired to the Agon program at `3UyUFeNsUYPpM6hMRf7H8wg3MKEXQ82rqnsXhZrUwgSD` on `devnet`
- Two keypairs with a small amount of SOL on devnet
- One allowlisted settlement token. Gateway-channel v1 uses official devnet USDC, mint `4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU`; resolve its `token_id` from `TokenRegistry` or `AGON_PROTOCOL_DEVNET_USDC_TOKEN_ID`.
