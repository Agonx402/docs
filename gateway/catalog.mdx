...
| `accessMode` | `"exact" \| "siwx" \| "agon-channel"` | x402 payment, wallet auth, or Agon channel authorization. See [Access modes](/gateway/access-modes). |
...
| `priceTokenAmount` | `string` (optional) | Six-decimal official devnet USDC amount, present for `agon-channel` routes. |
| `tokenMint` | `string` (optional) | Official devnet USDC mint for `agon-channel` routes. |
| `tokenId` | `number` (optional) | Protocol token ID for `agon-channel` routes, resolved from the live registry/config. |
...
5. If `accessMode: "agon-channel"`, send `X-Agon-Request-Id` and `AGON-COMMITMENT` with the next cumulative channel amount.
