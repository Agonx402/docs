...
for (const route of catalog.routes) {
  console.log(route.httpMethod, route.path, route.accessMode, route.priceUsd ?? route.priceTokenAmount ?? "");
}
```
...
- For packaged skills, CLI, MCP servers, and LLM-readable docs, see [Agentic tools](/reference/agentic-tools).
...
The hosted gateway is at `https://gateway.agonx402.com`.
