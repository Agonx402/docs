---
title: "Agon Gateway"
description: "A hosted gateway for x402, SIWX, and Agon channel-backed Solana infrastructure routes."
---

Agon Gateway is a hosted gateway that turns major Solana infrastructure providers and the Tokens API into permissionless HTTP endpoints. Clients do not need upstream API keys; they authenticate and pay through x402, SIWX, or Agon Protocol payment channels.
...
- **Use payment channels for agents.** `/v1/agon-channel/...` routes accept signed cumulative commitments against locked official devnet USDC collateral.
...
- **Non-custodial.** The gateway never holds your funds. x402 payments settle directly on Solana; channel collateral stays inside Agon Protocol.
- **Standard x402 where appropriate.** Anything that speaks x402 works against the x402 routes. Channel routes use Agon-specific tooling.
...
| Agon channel token | Official devnet USDC (`4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU`) |
| Catalog | `GET /v1/catalog` |
| Health | `GET /healthz` |

## What it is not

- **Not a custodian.** The gateway never holds client funds. x402 payments settle directly on Solana; channel collateral remains in Agon Protocol accounts.
- **Not the protocol itself.** The gateway is an application-layer resource server that can use x402 or Agon Protocol channels.
