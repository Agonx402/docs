...
description: "The HTTP headers the gateway reads and returns for x402, SIWX, and Agon channel flows."
---
...
Used by **channel-backed routes** (`accessMode: "agon-channel"`).

- Stable idempotency key for one gateway request.
- Required with `AGON-COMMITMENT`.
...
- Base64 JSON envelope containing the signed Agon cumulative commitment payload.
...
Channel-backed routes do not return `X-PAYMENT-RESPONSE` per request. Accepted commitments are settled later through Agon Protocol bundle settlement.
