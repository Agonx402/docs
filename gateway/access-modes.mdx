---
title: "Access Modes"
description: "x402 exact, wallet-authenticated SIWX, and Agon channel routes."
---
...
| `agon-channel` | `X-Agon-Request-Id` + `AGON-COMMITMENT` | Settled later through Agon Protocol | Channel-backed Solana RPC, DAS, and Helius Wallet routes |

The `exact` and `siwx` modes use the x402 request-challenge-retry pattern. The `agon-channel` mode uses an already-open Agon payment channel and a signed cumulative commitment per request.
...
## `agon-channel` - channel-backed gateway calls

Used by `/v1/agon-channel/...` routes. The agent opens a channel to the gateway merchant participant, locks official devnet USDC, and sends a signed cumulative commitment with each request.

Required request headers:

- `X-Agon-Request-Id`: stable idempotency key for the request.
- `AGON-COMMITMENT`: base64 JSON envelope containing the signed `agon-cmt-v5` cumulative commitment payload.
...
- **Payment-channel gateway calls** are best for agents that repeatedly call RPC, DAS, or wallet routes and want to amortize settlement with Agon Protocol instead of paying an on-chain transfer per request.

The x402 flows use standard x402 extensions, so any x402-compatible client library (including `@x402/fetch`) supports them out of the box. Channel-backed calls use Agon-specific protocol tooling; see [Agentic tools](/reference/agentic-tools).
